...
Use `import Anthropic from "npm:@anthropic-ai/sdk@latest"` (Deno import pattern).
  ...
  <verify>
The file should export the `classifyMaterials` function and `MaterialClassification` type. Verify the file has no syntax errors by checking it follows the same patterns as process-document/prompt.ts.
  </verify>
  <done>
...
**Important:** Use a higher similarity threshold (0.5) for auto-matching than the find_similar_material default (0.3). The function returns results above 0.3 for potential matches, but we only auto-link at 0.5+ to avoid false positives. This leaves room for manual matching later.
...
Should return 400 (quote not found), not 500 (function crash).
  ...
  <done>
normalize-materials Edge Function deployed. Accepts quote_id, classifies all unnormalized line items via AI, fuzzy-matches against existing materials (0.5 threshold for auto-match), creates new materials when no match found, sets material_id on all line items, records aliases. Handles errors per-item without failing the batch.
  </done>
</task>
