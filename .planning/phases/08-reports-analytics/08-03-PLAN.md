...
<task type="auto">
  ...
  <action>
    ...
    **Empty state:**
    If data is empty, show a centered message: "No category data available. Ensure materials are linked to categories."

    **Step 2: Integrate into ReportsPage.tsx**

    Add a second chart section below the PriceTrendChart card. Use a tab-like toggle to switch between "Supplier Comparison" and "Category Trends" views.
    ...
    - "supplier": Render `PriceTrendChart` (existing)
    - "category": Render `CategoryAggregateChart` with filtered data and categories from `useReportsCategories()`
  ...
  <done>
    CategoryAggregateChart renders average prices per category over time. ReportsPage has toggle buttons to switch between Supplier Comparison and Category Trends views. Both charts accept filtered data and support click-to-quote navigation. Build passes cleanly.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <what-built>
    ...
    - Data table below charts showing filtered results with quote links
  </what-built>
  <how-to-verify>
    ...
    7. Verify data table below chart shows filtered data with quote links
...
5. All 7 phase success criteria are met:
