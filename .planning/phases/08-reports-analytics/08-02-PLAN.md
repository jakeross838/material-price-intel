...
<task type="auto">
  <name>Task 2: Integrate PriceTrendChart into ReportsPage with quote navigation</name>
  <files>
    material-price-intel/src/pages/ReportsPage.tsx
  </files>
  <action>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-lg flex items-center gap-2">
          ...
          Price Trends
          {materialFilter !== "all" && selectedMaterialName && (
    ...
    **Do NOT add a data table.** The SearchPage already provides full tabular data access. This plan focuses solely on the chart visualization and click-to-quote navigation. Adding a redundant data table is out of scope for the 7 phase success criteria.
  </action>
  <verify>
    1. Run `cd material-price-intel && npx tsc --noEmit` -- no TypeScript errors
    2. Run `cd material-price-intel && npm run build` -- build succeeds
    ...
    5. Grep for "PriceTrendChart" in ReportsPage.tsx -- should appear in JSX
    6. Confirm there is NO data table in ReportsPage.tsx (no `<table>` or `<thead>` elements)
  </verify>
  <done>
    ReportsPage renders the PriceTrendChart with filtered data and click-to-quote navigation. The chart title shows the selected material name when filtered. A hint encourages material selection for best comparison. No data table -- SearchPage handles tabular access. Build passes cleanly.
  </done>
</task>
...
7. Chart adapts to filter state (specific material = cleanest view)
</verification>
