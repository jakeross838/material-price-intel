...
must_haves:
  ...
  key_links:
    - from: "QuoteDetailPage"
      to: "ReviewForm"
      via: "import and render (ReviewForm renders LineItemsEditor internally)"
...
- When `quote.is_verified === false` (review mode): Show a two-column side-by-side layout:
  ...
  - **Right column (50% width):** ValidationWarnings at top, then ReviewForm below. **ReviewForm renders LineItemsEditor internally** -- do NOT render LineItemsEditor separately as a sibling.
...
1. **Import new components** (note: NO LineItemsEditor import needed -- ReviewForm handles it):
   ```typescript
...
**IMPORTANT:** Keep ALL existing read-only rendering code. Wrap it in the `quote.is_verified` conditional. Do not delete any existing functionality.
  ...
  <verify>
    1. QuoteDetailPage imports ReviewForm, ValidationWarnings, ConfidenceBadge (NOT LineItemsEditor directly for rendering)
  ...
  <done>
    QuoteDetailPage shows side-by-side PDF + review form for unverified quotes. ReviewForm is the only review component rendered directly (it renders LineItemsEditor internally). Approved quotes show the original read-only view with a green "Approved" badge. Save and Approve actions are wired to the review hooks.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    1. Database migrations: pg_net extraction trigger (007) + approved status and review RPCs with org ownership (008)
    ...
    The full flow: Upload PDF -> auto-extraction via pg_net trigger -> document appears with status -> click to review -> see PDF + extracted data side-by-side -> edit any field -> approve -> status changes to approved.
  </what-built>
  <how-to-verify>
    **Pre-requisites:**
    1. Apply migrations 007 and 008 to Supabase (via SQL editor or `supabase db push`)
    2. Run `npm run dev` in `material-price-intel/`
    ...
    **Test the review flow:**
    ...
    5. Watch the RecentUploads list -- document should go from pending -> processing -> completed/review_needed (triggered automatically by pg_net)
...
3. ReviewForm is the only review component rendered directly (LineItemsEditor is internal)
4. Save and Approve buttons work correctly
5. Validation warnings are displayed
6. Low-confidence fields are highlighted
7. Dashboard shows document status counts
8. Dashboard shows "Needs Attention" section
9. RecentUploads shows approved status badge
10. Full end-to-end flow works: upload -> extract -> review -> approve
11. `npm run build` passes
</verification>

<success_criteria>
- Side-by-side view: PDF on left, extracted data on right (for unverified quotes)
- All extracted fields editable: quote details, line items, totals
