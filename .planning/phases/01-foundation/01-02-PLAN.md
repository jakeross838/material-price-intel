...
must_haves:
  truths:
    - "Database migrations execute against Supabase in order without errors"
    - "Schema supports structured material fields: species, dimensions, grade, treatment, UOM"
    - "Material categories are extensible beyond lumber via material_categories table and category_attributes JSONB column"
...
<task type="auto">
  ...
  <action>
    ...
    3. Create `supabase/migrations/002_schema.sql` with ALL tables from the architecture research. Use the EXACT schema from ARCHITECTURE.md with these v1 scope adjustments:
       ...
       e. `materials` -- id, organization_id, category_id, canonical_name, species, dimensions, grade, treatment, unit_of_measure (default 'piece'), description, aliases (TEXT[] default '{}'), category_attributes (JSONB DEFAULT '{}' -- for category-specific fields like window frame_material, glass_type, etc.), is_active (default true), created_at, updated_at. UNIQUE on (organization_id, canonical_name).
...
5. Material table has: species, dimensions, grade, treatment, unit_of_measure, aliases (TEXT[]), category_attributes (JSONB)
