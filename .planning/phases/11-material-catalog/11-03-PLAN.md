...
must_haves:
  ...
  key_links:
    ...
    - from: "MaterialImageGallery.tsx"
      to: "getMaterialImageDisplayUrl"
      via: "import from useMaterialImages"
      pattern: "getMaterialImageDisplayUrl"
---

<objective>
Build the reusable UI components for the material catalog. These are presentational components that receive data via props — they do NOT call hooks directly (except the image gallery for URL resolution). The parent page (Plan 04) wires hooks to these components.
...
Import `getMaterialImageDisplayUrl` from `@/hooks/useMaterialImages` for URL resolution — but ONLY if the image has a storage_path instead of image_url. Actually, since this component receives pre-resolved URLs via props, it should NOT import the hook. The parent resolves URLs before passing.

Actually, simplify: images prop already has image_url which is the display URL. Use image_url directly. If thumbnail_url exists, use it for the thumbnail strip (smaller/faster). No hook import needed.
