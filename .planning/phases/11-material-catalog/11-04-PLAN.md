...
<task type="auto">
  <name>Task 2: Create AdminCatalogPage and update routes</name>
  <files>material-price-intel/src/pages/AdminCatalogPage.tsx, material-price-intel/src/App.tsx</files>
  <action>
...
- Once a material is selected, show:
  ...
  3. **Quick scrape**: If the material has no images/documents, show a "Scrape from URL" option that calls the existing scrape-product Edge Function and maps the results to material_images and material_documents. This is a convenience bridge from the existing product_data scraper. Implement as a "Import from Product URL" button that:
     - Takes a URL input
...
Add imports for CatalogPage, CatalogDetailPage, AdminCatalogPage at top of App.tsx.
...
Also add a navigation link to the catalog admin in the AppLayout sidebar/nav. Check the layout component for where nav items are defined and add an entry for "Catalog" under the admin section, using the Palette or Grid3x3 icon from lucide.
  ...
  <done>AdminCatalogPage provides material image/document management and room-category mapping editor. App.tsx routes /catalog and /catalog/:id as public (outside ProtectedRoute), /admin/catalog as authenticated. Full build passes.</done>
</task>
