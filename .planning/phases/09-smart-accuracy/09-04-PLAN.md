...
must_haves:
  truths:
    - "All existing line items are re-classified with appropriate line_type values"
    - "Discount-like line items (description matches discount patterns) are reclassified from material to discount"
    - "Fee-like line items (surcharges, handling fees) are reclassified from material to fee"
    - "Subtotal-like line items are reclassified from material to subtotal_line"
    - "effective_unit_price is computed for all existing material items based on their discount fields"
    - "No data is lost -- raw_description, unit_price, and all original fields are preserved"
  artifacts:
