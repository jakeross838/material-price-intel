...
must_haves:
  truths:
    ...
    - "When user changes line_type to 'material', effective_unit_price is recomputed from unit_price and discount fields"
    - "When user changes line_type away from 'material', effective_unit_price is cleared to null"
  ...
  artifacts:
    - path: "material-price-intel/src/components/review/LineItemsEditor.tsx"
      provides: "Line item editor with line_type dropdown column, effective price display, and recomputation on reclassify"
      contains: "line_type"
  ...
  key_links:
    ...
    - from: "LineItemsEditor.tsx handleFieldChange"
      to: "effective_unit_price recomputation"
      via: "when line_type changes, recomputes or nulls effective_unit_price"
      pattern: "handleFieldChange.*line_type.*effective_unit_price"
    - from: "ReviewForm.tsx"
...
Purpose: Phase 9 requirement #6: "Review UI shows line item classifications and lets users reclassify if AI got it wrong." Users need to see what type each line is (material, discount, fee, etc.), be able to change it during review, and see effective prices on approved quotes. When reclassifying, the effective_unit_price must be recomputed client-side to keep data consistent.

Output: Updated LineItemsEditor with type dropdown and effective price recomputation, updated QuoteDetailPage with type badges and effective prices.
</objective>
...
<task type="auto">
  <name>Task 1: Add line_type dropdown, effective price recomputation on reclassify, and wire through ReviewForm</name>
  <files>
    material-price-intel/src/components/review/LineItemsEditor.tsx
    material-price-intel/src/components/review/ReviewForm.tsx
    material-price-intel/src/hooks/useQuoteReview.ts
...
export type EditableLineItem = {
  ...
  discount_pct?: number | null;          // NEW - needed for effective price recomputation
  discount_amount?: number | null;       // NEW - needed for effective price recomputation
};
```
...
f) **CRITICAL: Update `handleFieldChange` to recompute effective_unit_price when line_type changes.**

Replace the existing handleFieldChange function with:
```typescript
  ...
  if (field === "line_type") {
    ...
    if (raw === "material") {
      ...
      if (effectivePrice != null) {
        ...
        } else if (item.discount_amount != null && item.discount_amount > 0 && item.quantity != null && item.quantity > 0) {
          effectivePrice = effectivePrice - (item.discount_amount / item.quantity);
        }
        effectivePrice = Math.max(0, Math.round(effectivePrice * 10000) / 10000);
      }
      updated = updateItem(updated, index, "effective_unit_price", effectivePrice);
    } else {
...
This ensures:
- Changing discount -> material: computes effective_unit_price from unit_price and any discount fields
- Changing material -> fee/discount/note/subtotal: nulls out effective_unit_price (non-materials should not have one)
- Changing fee -> material: also computes effective_unit_price
...
- EditableLineItem type includes line_type, effective_unit_price, discount_pct, discount_amount
- handleFieldChange contains a `if (field === "line_type")` block that:
  - When raw === "material": computes effective_unit_price from unit_price/discount fields
  - When raw !== "material": sets effective_unit_price to null
...
- New rows default to line_type='material'
  ...
  <done>Review mode shows line_type dropdown per item with color coding, recomputes effective_unit_price on reclassification, and saves line_type through to the database</done>
</task>
...
setEditableLineItems(
  lineItems.map((item, index) => ({
    ...
    effective_unit_price: item.effective_unit_price,    // NEW
    discount_pct: item.discount_pct,                   // NEW - for reclassification recomputation
    discount_amount: item.discount_amount,             // NEW - for reclassification recomputation
  }))
);
```
...
- editableLineItems initialization includes line_type, effective_unit_price, discount_pct, discount_amount
- LineTypeBadge helper function exists with 5 type configs
  </verify>
  <done>Approved quote view shows line type badges (color-coded), effective price column, and dims non-material items. Review mode properly initializes line_type from stored data including discount fields for reclassification.</done>
</task>
...
- Review mode: changing line_type to 'material' recomputes effective_unit_price from unit_price/discount fields
- Review mode: changing line_type away from 'material' nulls effective_unit_price
...
3. Reclassifying to 'material' recomputes effective_unit_price; reclassifying away nulls it
4. Reclassification is persisted when saving
5. Effective unit price displayed when different from raw unit price
6. Visual distinction between material and non-material items
</success_criteria>
