...
The `stats` useMemo hook (lines 118-166) references `.unitPrice` in 5 specific locations that must change to `.effectiveUnitPrice`:

a) **Line 120** -- prices array construction:
```typescript
// Before:
...
b) **Line 128** -- older half prices for trend calculation:
```typescript
// Before:
const olderPrices = sorted.slice(0, mid).map((d) => d.unitPrice);
// After:
const olderPrices = sorted.slice(0, mid).map((d) => d.effectiveUnitPrice);
```

c) **Line 129** -- recent half prices for trend calculation:
```typescript
// Before:
const recentPrices = sorted.slice(mid).map((d) => d.unitPrice);
// After:
const recentPrices = sorted.slice(mid).map((d) => d.effectiveUnitPrice);
```

d) **Line 148** -- supplier average accumulation:
```typescript
// Before:
entry.total += d.unitPrice;
// After:
entry.total += d.effectiveUnitPrice;
```

These are ALL the `.unitPrice` references in ReportsPage.tsx. The rest of the file uses `stats.avg`, `stats.bestSupplier`, etc. which are derived values that will automatically use effective prices once the above 4 locations are updated.

**Important:** Do NOT change the `unitPrice` property in the ReportDataPoint type -- keep it for raw price display in tooltips. Only change the computation references to use `effectiveUnitPrice`.

Keep `unitPrice` available in the type for showing raw price in tooltips if desired, but use `effectiveUnitPrice` for all computed metrics and chart rendering.
  </action>
  <verify>
...
Read ReportsPage.tsx and verify specifically:
- Line ~120: `filteredData.map((d) => d.effectiveUnitPrice)` (not d.unitPrice)
- Line ~128: `sorted.slice(0, mid).map((d) => d.effectiveUnitPrice)` (not d.unitPrice)
- Line ~129: `sorted.slice(mid).map((d) => d.effectiveUnitPrice)` (not d.unitPrice)
- Line ~148: `entry.total += d.effectiveUnitPrice` (not d.unitPrice)
- No remaining `.unitPrice` in any useMemo computation (only in type definition is OK)
  </verify>
