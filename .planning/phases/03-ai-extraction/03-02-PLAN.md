...
<task type="auto">
  ...
  <action>
    ...
    **Add a `persistExtraction` async function:**
    ...
    Note: Line-item confidence from `ExtractedLineItem.confidence` is stored in `raw_extraction` JSONB (along with validation warnings), not in a dedicated column on `line_items`. The Phase 4 review UI will parse `raw_extraction` to highlight low-confidence line items. This avoids a schema migration for a field only needed during review.
